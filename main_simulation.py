...
def main():
    ...
    print("Cup position: ", [x_cup, y_cup, z_cup])
    #standoff position 
    # p = np.array([x_cup, y_cup, z_cup + 0.1])
    x_offset = 0
    if x_cup < robot_current_position[0]:
        x_offset = 0.02
    else:
        x_offset = -0.02

    y_offset = 0.15
    if y_cup > robot_current_position[1]:
        y_offset = -0.15

    p = np.array([x_cup + x_offset, y_cup + y_offset, z_cup + 0.075])
    ...
    R_standoff = np.matmul(rotZ(-(np.deg2rad(90) - np.atan2(x_cup - robot_current_position[0], robot_current_position[1] - y_cup))), rotation)[:3,:3]
    # R_standoff = np.matmul(rotZ(np.deg2rad(-90)), rotY(np.deg2rad(-90)))
    print("R:", R_standoff, flush=True)
    q_wxyz = mat2quat(R_standoff)
    q_xyzw = np.array([q_wxyz[1], q_wxyz[2], q_wxyz[3], q_wxyz[0]])

    # move just before the cup 
    ...
    if retval_ik is None:
        ...
        time.sleep(4)
        # move just before the cup 
        p = np.array([x_cup, y_cup, z_cup + 0.075])
